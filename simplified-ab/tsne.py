...
Provide ONLY the reflection."""
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print("API request failed for generating reflection.")
            return ""
    except Exception as e:
...
def traverse_claude_tree(
    node, chat_name, chat_id, messages, branch_points, branch_id="0"
):
    ...
    try:
        ...
        if len(node["children"]) > 1:
            for idx, child in enumerate(node["children"]):
                ...
                traverse_claude_tree(
                    child, chat_name, chat_id, messages, branch_points, new_branch_id
                )
...
def generate_shift_label(parent_messages, child_messages):
    # Prepare the prompt
    parent_text = "\n".join(
        [f"{msg['sender']}: {msg['text']}" for msg in parent_messages]
    )
    child_text = "\n".join(
        [f"{msg['sender']}: {msg['text']}" for msg in child_messages]
    )
...
Describe the shift in conversation direction in one concise label."""
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(
                f"Error generating shift label: {response.status_code}, {response.text}"
            )
